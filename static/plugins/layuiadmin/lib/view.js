...
layui.define(['laytpl', 'layer'], function (exports) {
    ...
    view.exit = function () {
        ...
        //跳转到登入页
        location.href = '/notepress-admin/templates/admin/admin_index.html';
    };
    ...
    view.req = function (options) {
        ...
        if (request.tokenName) {
            if (NP_Config.accessTokenInParam) {
        ...
        return $.ajax($.extend({
            ...
            , dataType: 'json'
            , crossDomain: true
            , global: false //设置此项为false为了避免触发自定义的全局ajax处理事件和layui的处理重复或冲突
            , xhrFields: {withCredentials: true}//带上跨域的cookie，解决request.getSession为null的问题
            , success: function (res) {
                ...
                //其它异常
                    ...
                    var error = [
                        '<cite><b>错误</b>：</cite> ' + (res[response.msgName] || '返回状态码异常')
                        , debug()
                    ].join('');
            ...
            , error: function (e, code) {
                ...
                if (e.responseJSON && e.responseJSON.message) {
                    errTxt = e.responseJSON.message
                }
                var error = [
                    '请求异常，请重试<br><cite>错误信息：</cite>' + errTxt
                    , debug()
                ].join('');
                view.error(error);
    ...
    //请求模板
    function loadTemplate(views, that, params) {
        $.ajax({
            url: views
            , type: 'get'
            , dataType: 'html'
        ...
        if (originViews === 'admin/login') {
            ...
            $.ajax({
                ...
                , success: function (resp) {
                    if (resp.code === NotePress.Constant.CODE_SUCCESS && resp.data.username) {
                        if (originViews.indexOf('theme/router') === 0) {
                            var _vs = originViews.split("/");
                            var _v = setter.themeViews + _vs[2] + "/" + _vs[3] + setter.engine;
                            loadTemplate(_v, that, params);
                        } else {
                            loadTemplate(views, that, params);
                        }
                    } else if (resp.code === NP_Config.response.statusCode.logout || resp.code === -2) {
                        view.exit();
                    } else {
                        alert(resp.message);
                    }
    ...
    Class.prototype.parse = function (html, refresh, callback) {
        ...
        for (var i = elemTemp.length; i > 0; i--) {
            (function () {
                ...
                var isJson = dataElem.attr("lay-json");
                if (isJson === undefined || isJson === false || isJson === "false" || !isJson) {
                    if (url !== '') {
                        url = NP_Config.domain + url;
                    }
                }
                ...
                if (headers === null || headers === '') {
                    ...
                    headers = layui.data(setter.tableName)[request.tokenName] || '';
                }
