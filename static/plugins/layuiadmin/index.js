...
layui.extend({
    ...
    var setter = layui.setter
        ...
        , entryPage = function (fn) {
            ...
            //独立页面
            if (isIndPage || pathURL === '/admin/login') { //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
                ...
                if (setter.interceptor) {
                    ...
                    if (!local[setter.request.tokenName] && (local[setter.request.tokenName] === null || local[setter.request.tokenName] === '')) {
                ...
                if (admin.pageType === 'console') { //后台主体页
                    ...
                    var accessToken = layui.data(setter.tableName)[request.tokenName];
                    if (accessToken !== null && accessToken !== '' && accessToken !== undefined) {
                        ...
                        location.hash = '/admin/login/redirect=' + encodeURIComponent(pathURL); //跳转到登入页
                    }
                }
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.code !== NotePress.Constant.CODE_SUCCESS || !resp.data) {
                layer.msg(resp.message + "，即将跳转至初始设置页面");
                setTimeout(function () {
